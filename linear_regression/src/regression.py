...
--output_path output \
--output_prefix result_single_lipid \
...
def process_args():
    ...
    parser.add_argument('--output_prefix', type=str, default=None, help='Output (result) file name')
    ...
    # Set up logger file
    fn_log = os.path.join(args.output_path, f'{args.output_prefix}.log')
...
def run_single_regression(df_data, phenotype, covars, condition, fn_output, permute=False, verbose=False,
                   ...
                   fn_permute_ses='output.permute_ses', indx=None, fn_model='output.model',
...
if __name__=='__main__':
    ...
    if not args.output_prefix: args.output_prefix = args.phenotype + '_' + '_'.join(args.covars) # Create default output file name if not provided
    ...
    fn_output = os.path.join(args.output_path, args.output_prefix+'.txt')
    if os.path.isfile(fn_output) and not args.overwrite:
        logging.info('# Output file exists: %s' % fn_output)
    ...
    fn_permute_pvals = os.path.join(args.output_path, args.output_prefix+'.permute_pvals')
    fn_permute_betas = os.path.join(args.output_path, args.output_prefix+'.permute_betas')
    fn_permute_ses = os.path.join(args.output_path, args.output_prefix+'.permute_ses')
    ...
    if not args.phenotype:
        ...
        df_output = pd.read_csv(fn_output, sep='\t')
        df_output['pval_fdr'] = fdrcorrection(df_output['pval'])[1]
        df_output.to_csv(fn_output, sep='\t', index=False)
        ...
        if args.plot:
            ...
            plot_lr_result(df_result=df_output, raw_pval_col='pval',
                           ...
                           output_fn_prefix=args.output_prefix)
