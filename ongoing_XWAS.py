...
def regressor(inputs,
    ...
    - reg_type: Default is 'OLS'. OLS (continuous) or LOG (logistic regression, binary)
    Return:
    ...
    if reg_type.upper() == 'OLS':
        ...
        # Matching inter number in scikit-learn
        # Set disp=0 to scilence fitting processilence
        result = reg.fit(maxiter=100, disp=0)
...
def regression_loop(inputs,
    ...
    - inputs: input file name (use .txt or .csv suffix) or dataframe.
            ...
            For example: (Can also be a tab-delimited file ending with .txt)
            sampleID,pheno,protein_1,protein_2,AGE,SEX,PEER1,PEER2
            patient_1,1,1.1,2.4,20,0,0.3,0.8
            patient_2,0,1.4,2.3,31,1,1.5,0.5
    - outcome: a single column name of dependent variable (y)
    ...
    for col in inputs.columns:
        if (col not in adj_covariates) and (col!=outcome):
            ...
            try:
                ...
                print(f'Something went wrong for: {col}. Skip this colum')
            count += 1
            if count%100 == 0:
    ...
    if p_adj != 'None':
        ...
        if p_adj == 'FDR':
            ...
            df_result[f'pval_{p_adj}_adj'] = df_result['pval'] * total
